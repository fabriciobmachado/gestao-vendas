...
@RestController
@RequestMapping("/produtos") // Define que todas as requisições para /produtos caem aqui
public class ProdutoController {

    @Autowired // Injeção de Dependência: O Spring entrega o repositório pronto para uso
    private ProdutoRepository produtoRepository;

    // 1. ENDPOINT PARA CADASTRAR UM PRODUTO (POST)
    @PostMapping
    public Produto criarProduto(@RequestBody Produto produto) {
        // O @RequestBody transforma o JSON que vem da web em um objeto Produto Java
    ...
    public Produto atualizarProduto(@PathVariable Long id, @RequestBody Produto produtoAtualizado) {
        // Buscamos o produto existente ou lançamos uma exceção se não encontrar
